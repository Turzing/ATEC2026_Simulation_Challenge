...
精度 (无 GT):
    - centroid_uv + depth_m 统一反投影
    - EE 臂关节动态外参 (蹲下/伸臂)
    - grasp_anchor 每帧重算 world
    - 抖动时冻结 grasp_offset, 站稳后 grasp_reliable
"""
...
def _read_arm_joints(obs) -> Optional[np.ndarray]:
    try:
        p = _to_numpy(obs["proprio"]).astype(np.float32).reshape(-1)
        j0 = PROPRIO_ARM_START  # proprio[24:30] = arm_joint1..6
...
def _object_summary(o: dict, cam: str) -> dict:
    ...
    if cam == "ee":
        for k in ("grasp_pos_world", "grasp_quat_world", "grasp_offset_robot"):
...
def _pick_nav_target(
    ...
    if _obj_dist(head_tgt) + 0.25 < _obj_dist(ee_tgt) and _obj_dist(head_tgt) > 1.35:
        if _nav_quality(head_tgt) > _nav_quality(ee_tgt) - 15.0:
            return "head", head_objs, head_tgt
    return "ee", ee_objs, ee_tgt
...
class RgbdPureDualPipeline:
    def __init__(self):
        ...
        print("[RgbdPureDual] ee=nav+grasp | head=nav-only")
    ...
    def process(self, obs, dt: float = 0.02, gt_robot_pos=None, gt_robot_yaw=None, **_) -> dict:
        """gt_robot_pos/yaw 可选 (师姐仿真 odometry); 其余 kwargs 忽略"""
        self.frame_count += 1
        ...
        nav_cam, nav_objs, nav_tgt = _pick_nav_target(ee_nav, head_nav, ee_objs, head_objs)
        ee_d = _obj_dist(ee_grasp or ee_nav)
        phase = "grasp" if (ee_grasp and ee_d < GRASP_PHASE_DIST_M) else "approach"
        grasp_tgt = _finalize_ee(ee_grasp or ee_nav, rp, ry, arm_q) if (ee_grasp or ee_nav) else None

        if phase == "grasp" and ee_nav:
            nav_cam, nav_objs, nav_tgt = "ee", ee_objs, ee_nav
