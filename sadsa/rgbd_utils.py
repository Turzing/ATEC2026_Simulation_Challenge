...
def compute_dynamic_ee_cam_pos(arm_joints) -> np.ndarray:
    ...
    # 经验线性近似 (B2-Piper piper 臂), 量级 ~10–20cm
    delta = np.array([
        0.14 * np.sin(dq[0]) + 0.10 * np.sin(dq[2]) + 0.06 * dq[3],
        0.06 * np.sin(dq[0]) + 0.04 * np.cos(dq[1]),
        0.10 * (np.cos(dq[1]) - 1.0) + 0.07 * np.sin(dq[3]) + 0.05 * dq[4],
    ], dtype=np.float32)
