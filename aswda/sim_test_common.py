...
class SimpleWalk:
    def __init__(self, device: str, policy_path: str = "", action_dim: int = 20):
        self.device = device
        self.action_dim = max(int(action_dim), 12)
        self.policy = None
        self.mode = "scripted"
    ...
    def set_cmd(self, vx, wz):
        ...
        self.last_cmd = (float(vx), float(wz))

    def get_action(self, obs, step: int, turn_bias: float = 0.0):
        if self.mode == "rl":
            return self._rl(obs)
        a = torch.zeros(1, self.action_dim, dtype=torch.float32, device=self.device)
        t = step * 0.02
    ...
    def _rl(self, obs):
        ...
        full = torch.zeros(1, self.action_dim, device=self.device, dtype=p.dtype)
...
class ManualKeyboard:
    def __init__(self, device: str, policy_path: str, action_dim: int = 20):
        self.walk = SimpleWalk(device, policy_path, action_dim=action_dim)
        ...
        try:
            ...
            print("[keyboard] OK — 必须在 Isaac Sim 窗口按 WASD (不是 OpenCV 窗)", flush=True)
        except Exception as e:
            print(f"[keyboard] FAILED: {e}", flush=True)
            print("[keyboard] 无法 WASD; 请确认从 Isaac 内启动且 Sim 窗口已获焦", flush=True)
